...
if (isset($_POST)) {
	...
	if(!is_numeric($parentID))
		return '<div id="message" class="error fade"><p>'. __('An error occured.', 'mypageorder').'</p></div>';
		
	if (isset($_POST['btnReturnParent'])) { 
		$returnParentID = $_POST['hdnParentID'];
		if(!is_numeric($returnParentID))
			return '<div id="message" class="error fade"><p>'. __('An error occured.', 'mypageorder').'</p></div>';
		
		$parentsParent = $wpdb->get_row( $wpdb->prepare("SELECT post_parent FROM $wpdb->posts WHERE ID = %d", $returnParentID), ARRAY_N);
		$parentID = $parentsParent[0];
	}
	elseif (isset($_POST['btnOrderPages'])) { 
		check_admin_referer('update_mypageorder_'.$parentID);
...
<form name="frmMyPageOrder" method="post" action="">
	<?php wp_nonce_field('update_mypageorder_'.$parentID); ?>
...
function mypageorder_updateOrder()
{
	if (isset($_POST['hdnMyPageOrder']) && $_POST['hdnMyPageOrder'] != "") { 
		...
		for($i = 0; $i < $result; $i++)
		{
			...
			if(!is_numeric($str))
				return '<div id="message" class="error fade"><p>'. __('An error occured, order has not been saved.', 'mypageorder').'</p></div>';
